...
def mobility(
    ...
    """Write empty mobility function."""
    cw.writer(fstream)
    cw.writer(
        fstream,
        "AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE amrex::Real"
        " specMob(const int specid, const amrex::Real Te,"
        " const amrex::Real ndens, const amrex::Real emag,"
        " const amrex::Real T)",
    )
    cw.writer(fstream, "{")
    cw.writer(fstream, f"amrex::Real mob = 0.0;")
    cw.writer(fstream, f"return(mob);")
    cw.writer(fstream, "}")

def diffusivity(
    fstream,
):
    """Write empty diffusivity function."""
    cw.writer(fstream)
    cw.writer(
        fstream,
        "AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE amrex::Real"
        " specDiff(const int specid, const amrex::Real Te,"
        " const amrex::Real ndens, const amrex::Real emag,"
        " const amrex::Real T)",
    )
    cw.writer(fstream, "{")
    cw.writer(fstream, f"amrex::Real diff = 0.0;")
    cw.writer(fstream, f"return(diff);")
    cw.writer(fstream, "}")
...
def production_rate(
    ...
    if n_reactions > 0:
        cw.writer(
            ...
            " const amrex::Real T, const amrex::Real Te, amrex::Real EN, amrex::Real * enerExch)",
        )
    else:
        cw.writer(
            fstream,
    ...
    if n_reactions > 0:
        ...
        cw.writer(fstream, cw.comment("Precalculating values for electron energy exchange evaluation"))
        cw.writer(fstream, "amrex::Real ne = sc[E_ID] * 6.02214085774e23;")
        cw.writer(fstream, "amrex::Real Ue = 1.5 * Te * ne * 1.380649e-23;")
        cw.writer(fstream)

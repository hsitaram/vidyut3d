...
void Vidyut::Evolve()
{
    ...
    for (int step = istep[0]; step < max_step && cur_time < stop_time; ++step)
    {
        ...
        implicit_solve_scalar(cur_time,dt[0],EDN_ID,Sborder,expl_src,eden_bc_lo,eden_bc_hi);

        if(elecenergy_solve)
        {
            update_explsrc_at_all_levels(EEN_ID, Sborder, flux, expl_src, cur_time);
            implicit_solve_scalar(cur_time,dt[0],EEN_ID, Sborder, expl_src,eenrg_bc_lo,eenrg_bc_hi);
        }
